...
#elif WAVE_TYPE == SAW_WAVE
static void saw_Wave_Task(void *args);
...
#elif WAVE_TYPE == SAW_WAVE
    xTaskCreate(saw_Wave_Task, "saw_Wave_Task", 4096, &frequency, 5, NULL);
#elif WAVE_TYPE == TRIANGLE_WAVE
...
static void cosine_Wave_Task(void *args)
{
    ...
    while (1)
    {
        output = (uint8_t)(127.0 * sin(6.2832 * frequency * time) + 128.0);
        time += step;
...
void saw_Wave_Task(void *args)
{
    ...
    while (1)
    {
        output = (uint8_t)(255.0 * saw(time / period));
        time += step;
        if (time >= period)
            time = 0;
...
static void triangle_Wave_Task(void *args)
{
    ...
    while (1)
    {
        output = (uint8_t)(255.0 * triangle(time / period));
        time += step;
        if (time >= period)
            time = 0;
